...
namespace PlayBox
{
	static class VariableHelper
	{
		...
		static void FindPlayBoxFunctions()
		{
			...
			foreach (Assembly assembly in ReflectionHelper.FindRelevantAssemblies())
			{
				...
				foreach (Type type in assembly.GetTypes())
					...
					{
						PlayBoxFunctionAttribute attribute = method.GetCustomAttribute<PlayBoxFunctionAttribute>();
						if (attribute == null)
							continue;
		...
		static int VariableSorting(Variable x, Variable y)
		{
			...
			result = string.Compare(x.NiceName, y.NiceName, StringComparison.Ordinal);
			return result;
		}
	}
}