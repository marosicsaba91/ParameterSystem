	...
	public class State : MonoBehaviour
	{
        ...
        */
		...
		public bool TryChangeSelectedState(State oldState, State newState)
		{
			if (newState == this)
				return false;
			if (!selectedStates.Contains(oldState))
				return false;
			if (selectedStates.Contains(newState))
				return false;
		...
		public bool TryRemoveDefault(State state)
		{
			...
			if (!defaultStates.Contains(state))
				return false;
			if (stateMachineType == StateMachineType.OneEnabled && defaultStates.Count <= 1)
				return false;
